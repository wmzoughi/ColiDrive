...
class _DashboardFournisseurState extends State<DashboardFournisseur> {
  ...
  Widget build(BuildContext context) {
    ...
    final orderService = Provider.of<OrderService>(context); // 👈 AJOUTER
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: dashboardService.salesChartData.map((data) {
                          return _buildChartBar(
                            ...
                            (data['value'] as double).clamp(20, 80),
                      ...
                      Center(
                        child: Text(
                          '${localizations.totalSales} (${localizations.currency})',
                          style: const TextStyle(
                            color: Color(0xFF8A9AA8),
                            fontSize: 14,
                          ),
                  ...
                  ...dashboardService.recentTransactions.map((transaction) {
                    return Padding(
                      ...
                      child: _buildTransactionCard(
                        ...
                        commandeRef: transaction['commandeRef'] ?? 'N/A',
                        montant: (transaction['montant'] as num?)?.toDouble() ?? 0.0,
                        statut: transaction['statut'] ?? 'pending',
                        timeInfo: transaction['timeInfo'] ?? localizations.timeAgo,
                        isDelayed: transaction['isDelayed'] ?? false,
  ...
  Widget _buildTransactionCard({
    ...
    Color getStatusColor() {
      if (statut == 'delivered') return const Color(0xFF2EC4B6);
      if (statut == 'pending') return Colors.orange;
      if (statut == 'cancelled') return Colors.red;
      return isDelayed ? const Color(0xFFE71D36) : const Color(0xFF2EC4B6);
    }
    ...
    IconData getStatusIcon() {
      if (statut == 'delivered') return Icons.check_circle;
      if (statut == 'pending') return Icons.hourglass_empty;
      if (statut == 'cancelled') return Icons.cancel;
      return isDelayed ? Icons.warning_amber_rounded : Icons.check_circle;
    }

    Color bgColor = getStatusColor().withOpacity(0.1);
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: Text(
                        '+${montant.toStringAsFixed(2)} ${localizations.currency}',
                        style: TextStyle(
                          color: getStatusColor(),
                          fontSize: 12,
                          fontWeight: FontWeight.w600,
